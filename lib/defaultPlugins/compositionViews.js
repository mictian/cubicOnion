...
module.exports = {
	...
	augmentNodeChildrenForCompositeViews: function (attributes_result, code_generation_context)
	{
		...
		var self = this.self
		,	node_children = code_generation_context.currentNodes;
		...
		{
			node_children.splice(
				code_generation_context.currentIndex+1
			,	0
			,	{
					type: 'handlebars'
	...
	extensionMethods: {
		...
		handlebarsCompositeView: function (code_generation_context)
		{
			...
			,	output_code = ''
			,	generate_incrementaldom = !!(code_generation_context.adapter && code_generation_context.adapter.name === 'ID');

			if (generate_incrementaldom)
			{
				output_code = '(function (){ '+
					'if (_.isFunction('+invokable_fn+')) ' +
						' { '
						+invokable_fn+'('+self.lastAttributesResult+');'+
						' } ' +
					'})();';
			}
			else
